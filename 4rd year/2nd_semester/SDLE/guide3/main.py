...
def run_study(lower_bound, upper_bound, step, runs, random_or_preferential, messages_to_propagate):
    ...
    for i in nodes:
        ...
        for _ in range(runs):
            ...
            neighbor_count_list = [len(list(graph.neighbors(node))) for node in graph.nodes]
            neighbor_results.append(sum(neighbor_count_list) / len(neighbor_count_list))
            diameter_results.append(nx.algorithms.distance_measures.diameter(graph))
            # Propagate messages
            for _ in range(messages_to_propagate):
                msgs_sent = propagate_message(graph)
                message_count_results.append(msgs_sent)

        neighbor_count_list = [len(list(graph.neighbors(r))) for r in graph.nodes]
        results[i] = {
